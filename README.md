...
| **JWT 鉴权** | 真实 JWT 签发/验证，登录 API + 前端接入，可选跳过进入演示模式 |
| **文件预览** | 图片/文本/PDF 在线预览，视频/音频播放，流式 Range 读取 |
| **暗色模式** | Ant Design 5 暗色主题，自动检测 `prefers-color-scheme`，持久化偏好 |
| **文件分享** | 带密码保护、过期时间、下载次数限制的外链分享 |
| **WebSocket** | 上传进度实时推送，替代轮询 |
| **管理后台** | 系统状态监控、审计日志持久化、一致性巡检 |
| **PostgreSQL** | 生产级数据库支持，PostgreSQL / SQLite 双适配 |
| **移动端适配** | 响应式布局，小屏卡片视图，批量操作栏固定 |
...
- PostgreSQL（可选，生产环境推荐；默认使用 SQLite）
