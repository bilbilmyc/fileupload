...
export default function BatchToolbar({
  ...
  return (
    <div className="mt-3 flex items-center gap-1 sm:gap-3 px-1 sm:px-2 py-2 bg-blue-50 dark:bg-blue-900/30 rounded-md sticky bottom-2 z-10 shadow-sm">
      ...
      <Dropdown menu={{ items: downloadItems }} trigger={['click']}>
        <Button icon={<DownloadOutlined />} size="small">
          <span className="hidden sm:inline">批量下载</span> <DownOutlined className="hidden sm:inline" />
        </Button>
      </Dropdown>

      <Button icon={<ScissorOutlined />} size="small" onClick={onBatchMove}>
        <span className="hidden sm:inline">批量移动</span>
      </Button>
      ...
      <Button icon={<TagOutlined />} size="small" onClick={onBatchTag}>
        <span className="hidden sm:inline">批量标记</span>
      </Button>
